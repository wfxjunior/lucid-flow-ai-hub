...
export function DocumentSignatureDialog({ 
  ...
  const getDocumentTitle = () => {
    switch (documentType) {
      case 'invoice':
        return document.title || `Invoice #${document.invoice_number || document.id?.slice(0, 8)}`
      case 'estimate':
        return document.title || `Estimate #${document.estimate_number || document.id?.slice(0, 8)}`
      case 'quote':
        return document.title || `Quote #${document.quote_number || document.id?.slice(0, 8)}`
      case 'contract':
        return document.title || `Contract #${document.id?.slice(0, 8)}`
      case 'workorder':
        return document.title || `Work Order #${document.work_order_number || document.id?.slice(0, 8)}`
      default:
