...
export function useSignNowIntegration() {
  ...
  const authenticate = async (): Promise<string> => {
    try {
      ...
      const tokenData: SignNowToken = data
      setAccessToken(tokenData.access_token)
      
      return tokenData.access_token
    } catch (error) {
      console.error('SignNow authentication error:', error)
      toast.error('Failed to authenticate with SignNow')
      throw error
    } finally {
      setIsLoading(false)
    }
  ...
  const uploadDocument = async (documentContent: string, fileName: string): Promise<UploadResult> => {
    try {
      ...
      toast.error('Failed to upload document to SignNow')
      throw error
    } finally {
      setIsLoading(false)
    }
  ...
  const uploadAndSendForSignature = async (
    ...
    try {
      ...
      console.log('Invite sent:', inviteResult)
