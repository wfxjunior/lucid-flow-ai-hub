...
export class PDFService {
  ...
  static async generateBidPDF(bid: any, businessData: BusinessData): Promise<void> {
    const element = this.createBidElement(bid, businessData)
    document.body.appendChild(element)
    
    try {
      await this.generateFromElement(element, {
        filename: `bid_${bid.bidNumber || bid.id}_${Date.now()}.pdf`
      })
    } finally {
      document.body.removeChild(element)
    }
  ...
  private static createBidElement(bid: any, businessData: BusinessData): HTMLElement {
    const element = document.createElement('div')
    element.style.cssText = `
      width: 210mm;
      min-height: 297mm;
    ...
    const bidNumber = bid.bidNumber || `BID-${String(bid.id).slice(0, 8)}`
    
    element.innerHTML = `
      <div style="border-bottom: 3px solid #2563eb; padding-bottom: 20px; margin-bottom: 30px;">
        <div style="display: flex; justify-content: space-between; align-items: flex-start;">
          <div style="flex: 1;">
            <h1 style="margin: 0; font-size: 24pt; color: #2563eb; font-weight: 700;">${businessData.companyName || 'Company Name'}</h1>
            <p style="margin: 8px 0 4px 0; color: #64748b; font-size: 10pt;">${businessData.companyAddress || ''}</p>
            <p style="margin: 4px 0; color: #64748b; font-size: 10pt;">${businessData.companyPhone || ''} ${businessData.companyEmail ? '• ' + businessData.companyEmail : ''}</p>
          </div>
          <div style="text-align: right; background: #f8fafc; padding: 20px; border-radius: 8px; border-left: 4px solid #2563eb;">
            <h2 style="margin: 0 0 8px 0; font-size: 20pt; color: #1e293b; font-weight: 600;">BID PROPOSAL</h2>
            <p style="margin: 4px 0; color: #64748b; font-size: 10pt; font-weight: 500;">#${bidNumber}</p>
            <p style="margin: 4px 0; color: #64748b; font-size: 10pt;">Date: ${bid.submittedDate ? new Date(bid.submittedDate).toLocaleDateString() : new Date().toLocaleDateString()}</p>
            ${bid.closingDate ? `<p style="margin: 4px 0; color: #dc2626; font-size: 10pt; font-weight: 500;">Closes: ${new Date(bid.closingDate).toLocaleDateString()}</p>` : ''}
          </div>
        </div>
      </div>
      ...
      <div style="margin-bottom: 40px;">
        <div style="background: #f8fafc; padding: 20px; border-radius: 8px; border-left: 4px solid #16a34a;">
          <h3 style="margin: 0 0 12px 0; color: #166534; font-weight: 600;">SUBMITTED TO:</h3>
          <p style="margin: 4px 0; font-size: 12pt; font-weight: 500;">${bid.client || 'Client Name'}</p>
          <p style="margin: 4px 0; color: #64748b;">Project: ${bid.projectName || 'Project Name'}</p>
        </div>
      </div>

      <div style="margin-bottom: 40px;">
        <h3 style="margin: 0 0 16px 0; color: #1e293b; font-weight: 600; border-bottom: 2px solid #e2e8f0; padding-bottom: 8px;">PROJECT DESCRIPTION</h3>
        <div style="background: #fefefe; padding: 20px; border: 1px solid #e2e8f0; border-radius: 6px;">
          <p style="margin: 0; line-height: 1.8;">${bid.description || 'Project description will be provided here.'}</p>
        </div>
      </div>

      <div style="margin-bottom: 40px;">
        <div style="background: linear-gradient(135deg, #2563eb 0%, #1d4ed8 100%); color: white; padding: 24px; border-radius: 8px; text-align: center;">
          <h3 style="margin: 0 0 8px 0; font-size: 14pt; font-weight: 500; opacity: 0.9;">TOTAL BID AMOUNT</h3>
          <p style="margin: 0; font-size: 32pt; font-weight: 700;">$${(bid.bidAmount || 0).toLocaleString()}</p>
        </div>
      </div>

      <div style="margin-bottom: 40px;">
        <h3 style="margin: 0 0 16px 0; color: #1e293b; font-weight: 600; border-bottom: 2px solid #e2e8f0; padding-bottom: 8px;">BID DETAILS</h3>
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
          <div style="background: #f8fafc; padding: 16px; border-radius: 6px;">
            <h4 style="margin: 0 0 8px 0; color: #374151; font-size: 10pt; font-weight: 600;">Status</h4>
            <p style="margin: 0; color: #16a34a; font-weight: 500; text-transform: capitalize;">${bid.status || 'Draft'}</p>
          </div>
          <div style="background: #f8fafc; padding: 16px; border-radius: 6px;">
            <h4 style="margin: 0 0 8px 0; color: #374151; font-size: 10pt; font-weight: 600;">Validity</h4>
            <p style="margin: 0; color: #64748b;">30 days from submission</p>
          </div>
        </div>
      </div>

      <div style="margin-top: 60px; border-top: 2px solid #e2e8f0; padding-top: 20px;">
        <div style="text-align: center; color: #64748b; font-size: 10pt;">
          <p style="margin: 8px 0; font-style: italic;">This bid proposal is valid for 30 days from the date of submission.</p>
          <p style="margin: 8px 0;">Thank you for considering our proposal. We look forward to working with you.</p>
        </div>
        
        <div style="margin-top: 40px; display: flex; justify-content: space-between;">
          <div style="width: 45%;">
            <div style="border-top: 1px solid #9ca3af; padding-top: 8px;">
              <p style="margin: 0; text-align: center; color: #64748b; font-size: 10pt;">Authorized Signature</p>
            </div>
          </div>
          <div style="width: 45%;">
            <div style="border-top: 1px solid #9ca3af; padding-top: 8px;">
              <p style="margin: 0; text-align: center; color: #64748b; font-size: 10pt;">Date</p>
            </div>
          </div>
        </div>
      </div>
