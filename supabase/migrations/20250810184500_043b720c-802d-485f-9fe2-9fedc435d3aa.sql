...
DROP POLICY IF EXISTS "Users can manage their own email senders" ON public.senders_email;
CREATE POLICY "Users can manage their own email senders"
...
DROP POLICY IF EXISTS "Users can manage their own comm messages" ON public.comm_messages;
CREATE POLICY "Users can manage their own comm messages"
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.templates WHERE user_id IS NULL AND name = 'Appointment Confirmation' AND context = 'appointments' AND language = 'en') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM public.templates WHERE user_id IS NULL AND name = 'Invoice Sent' AND context = 'billing' AND language = 'en') THEN
    INSERT INTO public.templates (user_id, name, context, language, subject, body_html, body_text, variables, is_default)
    VALUES (
